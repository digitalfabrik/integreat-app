...
import type { CitiesActionType, FetchCitiesRequestActionType } from '../../app/StoreActionType'
...
function * fetch (action: FetchCitiesRequestActionType): Saga<void> {
  try {
    ...
    const success: CitiesActionType = {type: `CITIES_FETCH_SUCCEEDED`, payload: payload}
    yield put(success)
  } catch (e) {
    const success: CitiesActionType = {type: `CITIES_FETCH_FAILED`, message: e.message}
    yield put(success)
  }
}
