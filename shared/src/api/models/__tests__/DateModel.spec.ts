...
describe('DateModel', () => {
  ...
  describe('formatRecurrenceEnd', () => {
    ...
    it('should report the real last occurrence date for a near-midnight recurrence', () => {
      const date = new DateModel({
        startDate: DateTime.fromISO('2025-07-04T23:30:00+02:00'),
        endDate: DateTime.fromISO('2025-07-04T23:30:00+02:00'),
        allDay: false,
        onlyWeekdays: false,
        recurrenceRule: rrulestr('DTSTART:20250704T233000\nRRULE:FREQ=WEEKLY;BYDAY=FR;UNTIL=20250725T233000'),
      })
      const interpolatingT = ((_: unknown, options: { date: string }) => `bis ${options.date}`) as unknown as TFunction

      expect(date.formatRecurrenceEnd('de', { t: interpolatingT })).toBe('bis Fr., 25. Juli 2025')
    })
  })
})
