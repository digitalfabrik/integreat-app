...
describe('createTrackingEndpoint', () => {
  ...
  it('should throw fetch error if fetch fails', async () => {
    ...
    return createTrackingEndpoint()
      .request(signal)
      .catch(e => expect(e.message).toContain('FetchError: Failed to load the request for the tracking endpoint. Das Internet ist kaputt!!!1!!!11elf!'))
  })
  ...
  it('should throw response error if response is not ok', async () => {
    ...
    return createTrackingEndpoint()
      .request(signal)
      .catch(e => expect(e.message).toContain('ResponseError: Failed to POST the request for the tracking endpoint with the url'))
  })

  it('should call the endpoint correctly', async () => {
    await createTrackingEndpoint().request(signal)
    expect(fetch).toHaveBeenCalledWith(JPAL_TRACKING_ENDPOINT_URL, expect.objectContaining({
      headers: {
