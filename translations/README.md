...
For this purpose, we use autogenerated index.ts barrel files: [src/translations/index.ts](src/translations/index.ts) and `src/override-translations/<build-config-name>/index.ts`.
In order to typecheck interpolation parameters and provide the translation values in the IDE,
we need to auto generate [src/resources.gen.ts](src/resources.gen.ts) with the resources as const.

Sync the index files and the resource types by running:
