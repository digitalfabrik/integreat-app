...
describe('Failure', () => {
  it('should render a retry button if tryAgain is passed', () => {
    const {
      getByTestId
    } = render(<Failure theme={lightTheme} tryAgain={() => {}} code={ErrorCode.UnknownError} t={key => key} />);
    expect(getByTestId('button-tryAgain')).toBeTruthy();
  });
  it('should not render a retry button if tryAgain is not passed', () => {
    const {
      queryByTestId
    } = render(<Failure theme={lightTheme} code={ErrorCode.UnknownError} t={key => key} />);
  ...
  it('should try again if button is pressed', () => {
    ...
    const {
      getByTestId
    } = render(<Failure theme={lightTheme} code={ErrorCode.UnknownError} tryAgain={tryAgain} t={key => key} />);
