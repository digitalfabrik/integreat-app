...
describe('openExternalUrl', () => {
  ...
  it('should open http urls in inapp browser', async () => {
    ...
    expect(sendTrackingSignal).toHaveBeenCalledWith({ signal: { name: OPEN_EXTERNAL_LINK_SIGNAL_NAME, url } })
    ...
    expect(Linking.openURL).not.toHaveBeenCalled()
    expect(sendTrackingSignal).toHaveBeenCalledWith({ signal: { name: OPEN_EXTERNAL_LINK_SIGNAL_NAME, url: url2 } })
  })
  ...
  it('should open non http urls with react native linking if possible', async () => {
    ...
    expect(InAppBrowser.open).not.toHaveBeenCalled()
    expect(sendTrackingSignal).toHaveBeenCalledWith({ signal: { name: OPEN_OS_LINK_SIGNAL_NAME, url: url } })
  })
