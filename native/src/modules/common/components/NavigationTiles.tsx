...
class NavigationTiles extends React.PureComponent<PropsType, StateType> {
  ...
  render() {
    ...
    return <TilesRow theme={theme}>
        {isScrollable && <AnchorIcon name='keyboard-arrow-left' language={language} navigationItemWidth={navigationItemWidth} _scrollView={this.state._scrollView} xPosition={xPosition} contentSizeDiff={contentSizeDiff} />}
        <ScrollView horizontal ref={this.setScrollViewRef} contentContainerStyle={{
        flexGrow: 1,
        justifyContent: 'space-around'
      }} style={{
        width: scrollViewWidth
      }} showsHorizontalScrollIndicator={false} pagingEnabled scrollEnabled snapToInterval={navigationItemWidth} decelerationRate='fast' bounces={false} onMomentumScrollEnd={this.onMomentumScrollEnd} snapToAlignment='center'>
          {tiles.map(tile => <NavigationTile key={tile.path} tile={tile} theme={theme} width={navigationItemWidth} />)}
        </ScrollView>
        {isScrollable && <AnchorIcon name='keyboard-arrow-right' language={language} navigationItemWidth={navigationItemWidth} _scrollView={this.state._scrollView} xPosition={xPosition} contentSizeDiff={contentSizeDiff} />}
      </TilesRow>;
  }
...
const NavigationTilesWithScrollableView = (props: {
  ...
  return <NavigationTiles navigationItemWidth={itemWidth} scrollViewWidth={scrollviewWidth} isScrollable={isScrollable} {...props} />;
};
