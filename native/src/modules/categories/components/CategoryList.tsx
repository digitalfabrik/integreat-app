...
class CategoryList extends React.Component<PropsType> {
  getListContent(listContent: ListContentModelType): React.ReactNode {
    ...
    const cacheDictionary = mapValues(listContent.files, (file: PageResourceCacheEntryStateType) => {
      return file.filePath.startsWith(RESOURCE_CACHE_DIR_PATH) ? file.filePath.replace(RESOURCE_CACHE_DIR_PATH, listContent.resourceCacheUrl) : file.filePath;
    });
    return <CategoryListContent content={listContent.content} language={language} navigateToLink={navigateToLink} cacheDictionary={cacheDictionary} lastUpdate={listContent.lastUpdate} theme={theme} />;
  }
  ...
  render() {
    ...
    return <>
        ...
        model,
        subCategories
      }) => <CategoryListItem key={model.path} category={model} language={language} subCategories={subCategories} query={query} theme={theme} onItemPress={onItemPress} />)}
      </>;
  }
