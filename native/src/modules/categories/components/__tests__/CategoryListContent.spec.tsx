...
jest.mock('../../../common/components/TimeStamp', () => ({
  lastUpdate
}: {
  lastUpdate: Moment;
}) => {
...
describe('CategoryListContent', () => {
  ...
  it('should display the content correctly', () => {
    ...
    const {
      getByText
    } = render(<CategoryListContent content={htmlContent} navigateToLink={navigateToLink} cacheDictionary={cacheDictionary} language='de' theme={lightTheme} />);
  ...
  it('should display arabic content correctly', () => {
    ...
    const {
      getByText
    } = render(<CategoryListContent content={htmlContent} navigateToLink={navigateToLink} cacheDictionary={cacheDictionary} language='ar' theme={lightTheme} />);
    expect(getByText(content1)).toBeTruthy();
    expect(getByText(content2)).toBeTruthy();
  });
  ...
  it('should display the last update timestamp correctly', () => {
    ...
    const {
      getByText
    } = render(<CategoryListContent content={content1} navigateToLink={navigateToLink} cacheDictionary={cacheDictionary} language='de' lastUpdate={lastUpdate} theme={lightTheme} />);
