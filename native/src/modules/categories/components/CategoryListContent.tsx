...
import type { HTMLNode } from "react-native-render-html";
...
const CategoryListContent = ({
    ...
    children = children && children.map((child, index) => {
      ...
      if (rawChild && rawChild.tagName === 'li') {
        if (rawChild.parentTag === 'ul') {
          ...
          prefix = listsPrefixesRenderers.ol(htmlAttribs, children, convertedCSSStyles, { ...passProps,
            index
          });
        }
      }

      return config.hasRTLScript(language) ? <View key={`list-${nodeIndex}-${index}-${key}`} style={{
        flexDirection: 'row'
      }}>
              <View style={{
          flex: 1
              ...
              {prefix}
            </View> : <View key={`list-${nodeIndex}-${index}-${key}`} style={{
        flexDirection: 'row'
      }}>
              {prefix}
              <View style={{
  ...
  return <SpaceBetween>
      <Container>
        <Html source={{
        html: content
      }} onLinkPress={onLinkPress} contentWidth={width} defaultTextProps={{
        selectable: true,
        allowFontStyling: true
      }} alterNode={alterResources} listsPrefixesRenderers={{
        ul: renderUnorderedListPrefix,
        ol: renderOrderedListPrefix
      }} renderers={{
        ul: renderLists,
        ol: renderLists
      }} baseFontStyle={{
        ...
        p: {
          textAlign: config.hasRTLScript(language) ? 'right' : 'left'
        },
        img: {
          align: config.hasRTLScript(language) ? 'right' : 'left'
        }
      }} />
        {lastUpdate && <LastUpdateContainer>
