...
const Header = (props: PropsType) => {
  ...
  const renderItem = (title: string, iconName?: string, show: "never" | "always", onPress: (() => void | Promise<void>) | null | undefined, accessibilityLabel: string): Node => {
    const buttonStyle = onPress ? {} : {
      color: theme.colors.textSecondaryColor
    };
    return <Item title={title} accessibilityLabel={accessibilityLabel} iconName={iconName} show={show} onPress={onPress} buttonStyle={buttonStyle} />;
  };
  ...
  return <BoxShadow theme={theme}>
      <Horizontal>
        <HorizontalLeft>
          {canGoBackInStack() ? <HeaderBackButton onPress={goBackInStack} labelVisible={false} /> : <Icon source={buildConfigAssets().appIcon} />}
          {routeCityModel && <HeaderText allowFontScaling={false} theme={theme}>
              {cityDisplayName()}
            </HeaderText>}
        </HorizontalLeft>
        <MaterialHeaderButtons cancelLabel={t('cancel')} theme={theme}>
          ...
          {!peeking && goToLanguageChange && renderItem(t('changeLanguage'), 'language', 'always', goToLanguageChange, t('changeLanguage'))}
