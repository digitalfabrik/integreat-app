...
describe('createNavigate', () => {
  ...
  it('should call navigateToCategory for dashboard route', () => {
    navigateTo({ route: DASHBOARD_ROUTE, cityContentPath, ...params }, key, forceRefresh)
  ...
  it('should call navigateToCategory for categories route', () => {
    navigateTo({ route: CATEGORIES_ROUTE, cityContentPath, ...params }, key, forceRefresh)
    assertOnlyCalled([navigateToCategory])
    expect(navigateToCategory).toHaveBeenCalledWith({
      dispatch,
      navigation,
  ...
  it('should call navigateToDisclaimer for disclaimer route', () => {
    navigateTo({ route: DISCLAIMER_ROUTE, ...params })
    assertOnlyCalled([navigateToDisclaimer])
    expect(navigateToDisclaimer).toHaveBeenCalledWith({ dispatch, navigation, ...params })
  })

  it('should call navigateToOffers for offer route', () => {
    navigateTo({ route: OFFERS_ROUTE, ...params })
    assertOnlyCalled([navigateToOffers])
    expect(navigateToOffers).toHaveBeenCalledWith({ dispatch, navigation, ...params })
  })

  it('should call navigateToEvents for events route', () => {
    navigateTo({ route: EVENTS_ROUTE, ...params, cityContentPath })
    expect(navigateToEvents).toHaveBeenCalledWith({ dispatch, navigation, ...params, cityContentPath })

    navigateTo({ route: EVENTS_ROUTE, ...params }, key, forceRefresh)
  ...
  it('should call navigateToNews for news route', () => {
    ...
    buildConfig.mockImplementationOnce(() => ({ featureFlags: { newsStream: true } }))
    navigateTo({ route: NEWS_ROUTE, ...params, newsType: LOCAL_NEWS_TYPE, newsId: '1234' }, key, forceRefresh)
  ...
  it('should not call navigateToNews if it is not enabled in build config', () => {
    // $FlowFixMe build config is a mock
    buildConfig.mockImplementationOnce(() => ({ featureFlags: { newsStream: false } }))
    navigateTo({ route: NEWS_ROUTE, ...params, newsType: LOCAL_NEWS_TYPE, newsId: '1234' }, key, forceRefresh)
    assertNotCalled(allMocks)
  })
  ...
  it('should call navigateToPois for pois route', () => {
    // $FlowFixMe build config is a mock
    buildConfig.mockImplementation(() => ({ featureFlags: { pois: true } }))

    navigateTo({ route: POIS_ROUTE, ...params, cityContentPath }, key, forceRefresh)
    expect(navigateToPois).toHaveBeenCalledWith({ dispatch, navigation, ...params, cityContentPath, forceRefresh, key })

    navigateTo({ route: POIS_ROUTE, ...params })
    expect(navigateToPois).toHaveBeenCalledWith({ dispatch, navigation, ...params, cityContentPath: null })
    assertOnlyCalled([navigateToPois], 2)
  })

  it('should not call navigateToPois if it is not enabled in build config', () => {
    // $FlowFixMe build config is a mock
    buildConfig.mockImplementationOnce(() => ({ featureFlags: { pois: false } }))
    navigateTo({ route: POIS_ROUTE, ...params, cityContentPath }, key, forceRefresh)
    assertNotCalled(allMocks)
  })
