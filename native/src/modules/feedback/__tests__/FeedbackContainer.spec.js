...
describe('FeedbackContainer', () => {
  ...
  it('should send feedback request on submit', async () => {
    const { getByText, findByText } = render(<FeedbackContainer
    ...
    expect(sendTrackingSignal).toHaveBeenCalledWith({
      signal: {
        name: SEND_FEEDBACK_SIGNAL_NAME, feedback: {
          positive: true,
          numCharacters: 0,
          contactMail: false
        }
  ...
  it('should send feedback request with comment and contact information on submit', async () => {
    ...
    const { getByText, findByText, getAllByDisplayValue } = render(<FeedbackContainer
      routeType={CATEGORIES_ROUTE}
      feedbackOrigin={'positive'}
      language={language}
      cityCode={city} />
