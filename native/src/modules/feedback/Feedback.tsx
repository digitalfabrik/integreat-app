...
const Feedback = (props: PropsType) => {
  const renderBox = (): React.ReactNode => {
    ...
    if (['idle', 'failed'].includes(sendingStatus)) {
      return <>
          ...
          <Input theme={theme} onChangeText={props.onCommentChanged} value={comment} multiline numberOfLines={3} autoFocus={!isSearchFeedback} />
          <DescriptionContainer theme={theme}>
            <Description theme={theme}>{t('contactMailAddress')}</Description>
            <Text>({t('optionalInfo')})</Text>
          </DescriptionContainer>
          <MailInput theme={theme} keyboardType='email-address' onChangeText={props.onFeedbackContactMailChanged} value={contactMail} />
          {sendingStatus === 'failed' && <Description theme={theme}>{t('failedSendingFeedback')}</Description>}
          <Button icon={<Icon name='send' size={15} color='black' style='material' />} titleStyle={{
          color: theme.colors.textColor
        }} buttonStyle={{
          backgroundColor: theme.colors.themeColor,
          marginTop: 15
        }} disabled={!isPositiveFeedback && !comment} onPress={props.onSubmit} title={t('send')} />
        </>;
      ...
      return <>
          ...
          <ThemedText theme={theme}>{t('feedback:thanksMessage', {
  ...
  return <ScrollView keyboardShouldPersistTaps='handled' style={{
