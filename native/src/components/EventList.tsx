...
const EventList = ({ events, regionModel, language, navigateTo, refresh }: EventListProps): ReactElement => {
  ...
  const sections = useMemo<EventSection[]>(() => {
    ...
    return groupEventsByDate(events).map(([key, events]) => {
      ...
      return { title: t($ => $.events.dateGroups[titleKey], params), data: events }
    })
  ...
  return (
    <ListContainer>
      <SectionList
        ...
        ListEmptyComponent={<ListEmptyComponent noItemsMessage={t($ => $.events.error.nothingFound)} />}
        refreshing={false}
        onRefresh={refresh}
