...
const renderJS = (
  ...
  (function handleIframes() {
    ...
    function showSettingsButton(element) {
      ...
      const buttonLabel = '${t($ => $.settings.title)}'
    ...
    function handleSupportedIframeSources(iframe, iframeSource) {
      ...
      if (externalSourcePermissions[iframeSource] === undefined) {
        const message = '${t($ => $.settings.externalSources.optIn)}'
        ...
        const message = '${t($ => $.settings.externalSources.allowed)}'
        showMessageWithSettings(message, iframeContainer, iframeSource)
      } else {
        const message = '${t($ => $.settings.externalSources.blocked)}'
        showMessageWithSettings(message, iframeContainer, iframeSource)
        iframe.remove()
      }
    }
