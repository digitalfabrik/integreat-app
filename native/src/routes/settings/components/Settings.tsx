...
const Settings = ({
  ...
  const setSetting = async (changeSetting: (settings: SettingsType) => $Shape<SettingsType>, changeAction?: (settings: SettingsType) => Promise<void>) => {
    if (!settings) {
      return;
    }
  ...
  const showSnackbar = (key: string) => dispatch({
  ...
  return <LayoutContainer>
      <SectionList keyExtractor={keyExtractor} sections={createSettingsSections({
      setSetting,
      t,
      languageCode,
      cityCode,
      ...
      settings,
      showSnackbar
    })} extraData={settings} renderItem={renderItem} renderSectionHeader={renderSectionHeader} ItemSeparatorComponent={ThemedItemSeparator} SectionSeparatorComponent={ThemedItemSeparator} stickySectionHeadersEnabled={false} />
    </LayoutContainer>;
};
