...
import { createOffersEndpoint, createSprungbrettJobsEndpoint, NotFoundError, OFFERS_ROUTE, SPRUNGBRETT_OFFER_ROUTE, SprungbrettJobModel } from "api-client";
...
const SprungbrettOfferContainer = ({
  route,
  navigation,
  theme,
  t
}: SprungbrettPropsType) => {
  ...
  if (jobsError || cityModel && !cityModel.offersEnabled) {
    const error = jobsError || new NotFoundError({
      type: 'category',
      id: 'offers',
      city: cityCode,
      language: languageCode
    });
    return <LayoutedScrollView refreshControl={<RefreshControl onRefresh={refresh} refreshing={loading} />}>
        <FailureContainer code={fromError(error)} tryAgain={refresh} />
      </LayoutedScrollView>;
  }

  return <LayoutedScrollView refreshControl={<RefreshControl onRefresh={refresh} refreshing={loading} />}>
      {jobs && <SprungbrettOffer title={title} jobs={jobs} t={t} theme={theme} language={languageCode} navigateToFeedback={navigateToFeedback} />}
    </LayoutedScrollView>;
};

export default withTranslation<$Diff<SprungbrettPropsType, {
  theme: ThemeType;
}>>('sprungbrett')(withTheme<SprungbrettPropsType>(SprungbrettOfferContainer));