...
const DisclaimerContainer = ({
  ...
  const request = useCallback(async (apiUrl: string) => await createDisclaimerEndpoint(apiUrl).request({
    city: cityCode,
  ...
  const {
    ...
    loading,
    refresh
  } = useLoadFromEndpoint<PageModel>(request);
  ...
  return <LayoutedScrollView refreshControl={<RefreshControl onRefresh={refresh} refreshing={loading} />}>
      {disclaimer && resourceCacheUrl && <Disclaimer resourceCacheUrl={resourceCacheUrl} disclaimer={disclaimer} theme={theme} navigateToLink={navigateToLinkProp} language={languageCode} />}
      <SiteHelpfulBox navigateToFeedback={navigateToFeedback} theme={theme} />
    </LayoutedScrollView>;
};

export default connect<PropsType, OwnPropsType, _, _, _, _>(mapStateToProps)(withTheme<DisclaimerPropsType>(DisclaimerContainer));