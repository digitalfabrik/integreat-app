...
const OffersContainer = ({
  ...
  const request = useCallback(async (apiUrl: string) => await createOffersEndpoint(apiUrl).request({
    city: cityCode,
  ...
  const {
    ...
    loading,
    refresh
  } = useLoadFromEndpoint<Array<OfferModel>>(request);
  ...
  if (offersError || cityModel && !cityModel.offersEnabled) {
    const error = offersError || new NotFoundError({
      type: 'category',
      id: 'offers',
      city: cityCode,
      language: languageCode
  ...
  return <LayoutedScrollView refreshControl={<RefreshControl onRefresh={refresh} refreshing={loading} />}>
      {offers && <Offers offers={offers} navigateToOffer={navigateToOffer} theme={theme} t={t} navigateToFeedback={navigateToFeedback} language={languageCode} />}
    </LayoutedScrollView>;
};

export default withTranslation<$Diff<OffersPropsType, {
  theme: ThemeType;
