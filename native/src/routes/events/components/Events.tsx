...
const Events = ({
  ...
  const renderEventListItem = (event: EventModel) => {
    return <EventListItem key={event.path} event={event} cityCode={cityModel.code} language={language} theme={theme} navigateTo={navigateTo} />;
  };
  ...
  if (path) {
    ...
    if (event) {
      ...
      return <Page content={event.content} title={event.title} lastUpdate={event.lastUpdate} language={language} files={files} theme={theme} resourceCacheUrl={resourceCacheUrl} navigateToLink={navigateToLink} navigateToFeedback={createNavigateToFeedbackForEvent(event)}>
          <>
            <PageDetail identifier={t('date')} information={event.date.toFormattedString(formatter)} theme={theme} language={language} />
