...
const Pois = ({ pois: allPois, cityModel, route, navigation }: PoisProps): ReactElement => {
  ...
  const [initialPoiListScrollIndex, setInitialPoiListScrollIndex] = useState(0)
  ...
  const selectPoi = (poi: PoiModel, index: number) => {
    setDeselectOnBackNavigation(true)
    navigation.setParams({ slug: poi.slug })
    setInitialPoiListScrollIndex(index)
  }
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <PoisBottomSheet
        ...
        initialScrollIndex={initialPoiListScrollIndex}
      />
    </View>
  )
}
