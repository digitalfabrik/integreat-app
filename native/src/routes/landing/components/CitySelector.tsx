  ...
  _renderFilteredLocations(cities: Array<CityModel>): React.ReactNode {
    ...
    return transform(groups, (result, cities, key) => {
      result.push(<CityGroupContainer key={key}>
            ...
            {cities.map(city => <CityEntry key={city.code} city={city} filterText={this.props.filterText} navigateToDashboard={this.props.navigateToDashboard} theme={this.props.theme} />)}
          </CityGroupContainer>);
    }, []);
  }
    ...
    if (location?.status === 'ready') {
      ...
      if (nearbyCities.length > 0) {
        return <CityGroupContainer>
            ...
            {nearbyCities.map(city => <CityEntry key={city.code} city={city} filterText={filterText} navigateToDashboard={navigateToDashboard} theme={theme} />)}
          </CityGroupContainer>;
      } else {
        return <CityGroupContainer>
            <CityGroup theme={theme}>{t('nearbyPlaces')}</CityGroup>
      ...
      return <CityGroupContainer>
          ...
          <NearbyMessageContainer>
            ...
            <RetryButtonContainer>
              {retryDetermineLocation && <Button icon={<Icon name='refresh' size={30} color={theme.colors.textSecondaryColor} style='material' />} title='' type='clear' onPress={retryDetermineLocation} accessibilityLabel={t('refresh')} accessibilityRole='button' />}
            </RetryButtonContainer>
          </NearbyMessageContainer>
        </CityGroupContainer>;
    }
