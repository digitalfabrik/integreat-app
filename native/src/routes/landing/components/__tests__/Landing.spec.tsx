...
describe('Landing', () => {
  ...
  it('should only show non-live cities', () => {
    ...
    const {
      getByText,
      queryByText
    } = render(<ThemeProvider theme={buildConfig().lightTheme}>
        <Landing cities={cities} language={language} t={key => key} theme={buildConfig().lightTheme} navigateToDashboard={navigateToDashboard} clearResourcesAndCache={clearResourcesAndCache} />
      </ThemeProvider>);
  ...
  describe('nearby locations', () => {
    ...
    it('should determine location and show no nearby locations if there are none', async () => {
      mockCheckLocationPermission.mockImplementationOnce(() => RESULTS.GRANTED);
      Geolocation.getCurrentPosition.mockImplementationOnce(setPosition => setPosition({
        coords: {
          longitude: 0,
          latitude: 0
        }
