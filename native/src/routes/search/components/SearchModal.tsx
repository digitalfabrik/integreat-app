...
class SearchModal extends React.Component<PropsType, SearchStateType> {
  ...
  findCategories(categories: CategoriesMapModel): Array<ListEntryType> {
    ...
    const categoriesWithTitle = categoriesArray.filter(category => normalizeSearchString(category.title).includes(normalizedFilter) && !category.isRoot()).map((category: CategoryModel): ListEntryType => ({
      model: {
    ...
    const categoriesWithContent = categoriesArray.filter(category => !normalizeSearchString(category.title).includes(normalizedFilter) && !category.isRoot()).map((category: CategoryModel): ListEntryType => {
    ...
    }).filter(category => category.model.contentWithoutHtml && category.model.contentWithoutHtml.length > 0 && normalizeSearchString(category.model.contentWithoutHtml).includes(normalizedFilter)).sort((category1, category2) => category1.model.title.localeCompare(category2.model.title));
    ...
    return <ScrollView contentContainerStyle={{
      flexGrow: 1
        ...
        minHeight: minHeight
          ...
          <CategoryList categories={filteredCategories} navigateToLink={navigateToLink} query={query} onItemPress={this.onItemPress} theme={theme} language={language} />
        </View>
        {filteredCategories.length === 0 && <>
            <SadIconContainer source={SadIcon} />
            <Heading theme={theme}>{t('feedback:nothingFound')}</Heading>
          </>}
        <FeedbackContainer routeType={SEARCH_ROUTE} isSearchFeedback isPositiveFeedback={false} language={language} cityCode={cityCode} query={query} theme={theme} />
      </ScrollView>;
  };
  ...
  render() {
    ...
    return <Wrapper theme={theme}>
        <SearchHeader theme={theme} query={query} closeSearchBar={this.onClose} onSearchChanged={this.onSearchChanged} t={t} />
