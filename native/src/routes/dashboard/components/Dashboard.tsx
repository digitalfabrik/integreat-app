...
import { CATEGORIES_ROUTE, EVENTS_ROUTE, LOCAL_NEWS_TYPE, NEWS_ROUTE, OFFERS_ROUTE, POIS_ROUTE, TU_NEWS_TYPE } from "api-client/src/routes";
...
class Dashboard extends React.Component<PropsType> {
  ...
  render() {
    ...
    return <SpaceBetween>
        {navigationTiles.length > 1 ? <NavigationTiles tiles={navigationTiles} theme={theme} language={language} /> : <Spacing />}
        <Categories stateView={stateView} resourceCache={resourceCache} resourceCacheUrl={resourceCacheUrl} language={language} cityModel={cityModel} theme={theme} navigateTo={navigateTo} navigateToFeedback={navigateToFeedback} navigateToLink={navigateToLink} />
      </SpaceBetween>;
  }
