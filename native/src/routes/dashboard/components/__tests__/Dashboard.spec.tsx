...
describe('Dashboard', () => {
  ...
  const stateView = new CategoriesRouteStateView(categoryLeaf.path, {
  ...
  const createCityModel = (events: boolean, offers: boolean, local: boolean, tunews: boolean, pois: boolean) => new CityModel({
    name: 'Stadt Augsburg',
    code: 'augsburg',
    live: true,
  ...
  const mockBuildConfig = (pois: boolean, newsStream: boolean) => {
    ...
    buildConfig.mockImplementation(() => ({ ...previous,
  ...
  const renderDashboard = (cityModel: CityModel) => <Dashboard navigateToLink={navigateToLink} navigateToFeedback={navigateToFeedback} navigateTo={navigateTo} theme={theme} language={language} cityModel={cityModel} stateView={stateView} resourceCache={resourceCache} resourceCacheUrl={resourceCacheUrl} t={t} />;
