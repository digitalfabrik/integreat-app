...
const JpalTracking = (props: PropsType) => {
  ...
  useEffect(() => {
    if (routeTrackingCode) {
      appSettings.setJpalTrackingCode(routeTrackingCode).then(() => loadSettings()).catch(() => {
        setError(true);
        loadSettings();
      });
    }
  ...
  const toggleTrackingEnabled = () => {
    setSettings({ ...settings,
      jpalTrackingEnabled: !settings.jpalTrackingEnabled
    });
  ...
  return <LayoutContainer>
      ...
      padding: 40
        ...
        <DescriptionContainer onPress={toggleTrackingEnabled}>
          ...
          <Switch thumbColor={props.theme.colors.themeColor} trackColor={{
          true: props.theme.colors.themeColor
        }} value={settings.jpalTrackingEnabled} onValueChange={toggleTrackingEnabled} testID='switch' />
        </DescriptionContainer>

        <ThemedText theme={props.theme}>{t('trackingCode')}</ThemedText>
        <Input value={settings.jpalTrackingCode} onChangeText={setTrackingCode} theme={theme} editable={settings.jpalTrackingEnabled} testID='input' />
      </View>
    </LayoutContainer>;
};
