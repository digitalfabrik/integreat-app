import type { CategoryRouteStateType, CitiesStateType, LanguageResourceCacheStateType, LanguagesStateType, ResourceCacheStateType, StateType } from "../../../../modules/app/StateType";
...
describe('CategoriesContainer', () => {
  ...
  const prepareState = (routeState: CategoryRouteStateType | null | undefined, {
    switchingLanguage,
    cities,
    languages,
    resourceCacheState
  ...
  const models = reduce(categoriesMap.toArray(), (acc, model) => ({ ...acc,
    [model.path]: model
  }), {});
  const children = reduce([rootCategory, ...categoriesMap.getChildren(rootCategory)], (acc, model) => ({ ...acc,
    [model.path]: categoriesMap.getChildren(model).map(child => child.path)
  }), {});
