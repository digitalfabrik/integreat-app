...
class Pois extends React.Component<PropsType> {
  ...
  renderPoiListItem = (cityCode: string, language: string) => (poi: PoiModel) => {
    ...
    return <PoiListItem key={poi.path} poi={poi} language={language} theme={theme} navigateToPois={this.navigateToPois(cityCode, language, poi.path)} />;
  };
  ...
  render() {
    ...
    if (path) {
      ...
      if (poi) {
        ...
        return <Page content={poi.content} title={poi.title} lastUpdate={poi.lastUpdate} language={language} files={files} theme={theme} resourceCacheUrl={resourceCacheUrl} navigateToLink={navigateToLink} navigateToFeedback={this.createNavigateToFeedbackForPoi(poi)}>
            <>
    ...
    return <SpaceBetween>
        <View>
          ...
          <List noItemsMessage={t('currentlyNoPois')} items={sortedPois} renderItem={this.renderPoiListItem(cityCode, language)} theme={theme} />
        </View>
