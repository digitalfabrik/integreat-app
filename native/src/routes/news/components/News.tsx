...
const News = (props: PropsType) => {
  ...
  const rendersNewsListItem = useCallback((cityCode: string, language: string) => ({
    ...
    return <NewsListItem key={item.id} newsItem={item} language={language} theme={theme} isTunews={selectedNewsType === TU_NEWS_TYPE} navigateToNews={navigateToNews} t={t} />;
  ...
  if (newsId) {
    ...
    if (selectedNewsItem) {
      return <NewsDetail newsItem={selectedNewsItem} theme={theme} language={language} navigateToLink={navigateToLink} />;
    } else {
      const error = new NotFoundError({
  ...
  return <View style={{
    flex: 1
      ...
      <NewsList renderNoItemsComponent={renderNoItemsComponent} items={news} isFetchingMore={isFetchingMore} fetchMoreItems={fetchMoreNews} renderItem={rendersNewsListItem(cityModel.code, language)} navigateTo={navigateTo} selectedNewsType={selectedNewsType} newsId={newsId} routeKey={routeKey} cityCode={cityModel.code} language={language} />
    </View>;
};

const TranslatedWithThemeNewsList = withTranslation<$Diff<PropsType, {
  theme: ThemeType;
