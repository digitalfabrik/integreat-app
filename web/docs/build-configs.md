...
Build configs are used in two different places: In the TypeScript code during runtime and in webpack during the build process.
...
The build-config is loaded by vite during [bundling and compiling the app](../vite.config.ts).
It is then passed as `__BUILD_CONFIG__` environment variable to the TypeScript code.
