...
describe('TunewsList', () => {
  ...
  const renderItem = (language: string) => ({
    ...
    content,
    date
  }: TunewsModel, city: string) => <NewsListItem title={title} content={content} timestamp={date} type={TU_NEWS} key={id} link={link} t={t} formatter={new DateFormatter(language)} />;
  ...
  it('should have two NewsListItem', () => {
    const tunewsList = shallow(<TunewsList language={language} items={tunews} renderItem={renderItem(language)} city={city} fetchMoreTunews={() => {}} hasMore isFetching={false} noItemsMessage={t('currentlyNoTunews')} />).dive();
  ...
  it('should render "currentlyNoTunews" if the items is an empty array and hasMore is false', () => {
    const tunewsList = shallow(<TunewsList language={language} items={[]} renderItem={renderItem(language)} city={city} fetchMoreTunews={() => {}} hasMore={false} isFetching={false} noItemsMessage={t('currentlyNoTunews')} />).dive();
