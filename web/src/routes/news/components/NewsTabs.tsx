...
const StyledTabs: StyledComponent<{}, ThemeType, any> = styled.div`
  display: flex;
  padding-top: 45px;
  padding-bottom: 40px;
`;
...
class NewsTabs extends React.PureComponent<PropsType> {
  render() {
    ...
    const localNewsPath = new LocalNewsRouteConfig().getRoutePath({
      city,
      language
    });
    const tunewsPath = new TunewsRouteConfig().getRoutePath({
      city,
      language
    });
    return <>
