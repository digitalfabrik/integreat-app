...
`;
const ReadMoreLink: StyledComponent<{}, ThemeType, any> = styled(({
  type,
  ...props
}) => <Link {...props} />)`
  align-self: flex-end;
  ...
  theme,
  type
}) => type === LOCAL_NEWS ? theme.colors.themeColor : theme.colors.tunewsThemeColor};
  color: ${({
  theme,
  type
}) => type === LOCAL_NEWS ? theme.colors.themeColor : theme.colors.tunewsThemeColor};
  font-weight: 600;
`;
const Title: StyledComponent<{}, ThemeType, any> = styled.h3`
  margin-bottom: 0;
  font-family: Raleway;
  font-size: 18px;
  font-weight: 700;
`;
const Body: StyledComponent<{}, ThemeType, any> = styled.p`
  font-size: 16px;
  line-height: 1.38;
`;
const StyledNewsListItem: StyledComponent<{}, ThemeType, any> = styled.div`
  padding-bottom: 2px;
