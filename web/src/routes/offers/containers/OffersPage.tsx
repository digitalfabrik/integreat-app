  ...
  toTileModels(offers: Array<OfferModel>): Array<TileModel> {
    ...
    return offers.map(offer => {
      ...
      if (offer.alias === SPRUNGBRETT_OFFER) {
        path = new SprungbrettRouteConfig().getRoutePath({
          city,
          language
        });
      } else if (offer.alias === WOHNEN_OFFER) {
        path = new WohnenRouteConfig().getRoutePath({
          city,
          language
        });
      }
...
export default connect<$Diff<PropsType, {
  t: TFunction;
}>, OwnPropsType, _, _, _, _>(mapStateToProps, () => ({}))(withTranslation<PropsType>('offers')(OffersPage));