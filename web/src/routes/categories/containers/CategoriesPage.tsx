...
const getBreadcrumb = (category: CategoryModel, cityName: string) => {
  ...
  return new BreadcrumbModel({
    ...
    node: <Link to={category.path} key={category.path}>
        {title}
      </Link>
  });
};
...
export const CategoriesPage = ({
  ...
  if (categoryModel) {
    ...
    const ancestorBreadcrumbs = categories.getAncestors(categoryModel).map(categoryModel => getBreadcrumb(categoryModel, cityName));
    return <div>
        <Breadcrumbs ancestorBreadcrumbs={ancestorBreadcrumbs} currentBreadcrumb={getBreadcrumb(categoryModel, cityName)} direction={uiDirection} />
...
export default connect<$Diff<PropsType, {
  t: TFunction;
}>, OwnPropsType, _, _, _, _>(mapStateToProps, () => ({}))(withTranslation<PropsType>('layout')(CategoriesPage));