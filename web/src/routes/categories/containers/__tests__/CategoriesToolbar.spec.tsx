...
describe('CategoriesToolbar', () => {
  ...
  it('should render nothing, if category cannot be found', () => {
    const component = shallow(<CategoriesToolbar categories={categories} location={createLocation({
      type: 'INVALID_ROUTE',
      payload: {},
      pathname: 'invalid_path'
    })} t={t} openFeedbackModal={() => {}} viewportSmall />);
    expect(component.getElement()).toBeNull();
  });
  it('should render Toolbar, if category can be found', () => {
    const component = shallow(<CategoriesToolbar viewportSmall categories={categories} location={createLocation({
