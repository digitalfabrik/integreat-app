...
function findComponent(wrapper: ShallowWrapper<any>, name: string): ShallowWrapper<any> {
  return wrapper.findWhere(n => n.name()?.endsWith(name));
}

describe('CategoryEntry', () => {
  it('should render and match snapshot', () => {
    const wrapper = shallow(<CategoryEntry theme={lightTheme} contentWithoutHtml={null} category={category} subCategories={[childCategory]} />).dive();
  ...
  it('should replace empty thumbnail', () => {
    const wrapper = shallow(<CategoryEntry theme={lightTheme} contentWithoutHtml={null} category={noThumbCategory} subCategories={[childCategory]} />).dive();
  ...
  describe('getMatchedContent', () => {
    it('should return the highlighted match item', () => {
      ...
      const numWords = 3;
      const wrapper = shallow(<CategoryEntry category={category} theme={lightTheme} contentWithoutHtml={category.content} query={query} subCategories={[]} />);
