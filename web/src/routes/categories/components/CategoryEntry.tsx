...
`;
const SubCategory: StyledComponent<{}, ThemeType, any> = styled.div`
  text-align: end;
...
`;
const CategoryThumbnail: StyledComponent<{}, ThemeType, any> = styled.img`
  width: 40px;
  height: 40px;
  flex-shrink: 0;
  padding: 8px;
  object-fit: contain;
`;
const CategoryListItem: StyledComponent<{}, ThemeType, any> = styled.div`
  display: flex;
...
`;
const SubCategoryCaption: StyledComponent<{}, ThemeType, any> = styled(CategoryListItem)`
  margin: 0 15px;
  padding: 8px 0;
  border-bottom: 1px solid ${props => props.theme.colors.themeColor};
`;
const ContentMatchItem: StyledComponent<{}, ThemeType, any> = styled(Highlighter)`
...
type PropsType = {
  ...
  contentWithoutHtml: string | null | undefined;
...
export class CategoryEntry extends React.PureComponent<PropsType> {
  ...
  renderSubCategories(): Array<React.ReactNode> {
    ...
    return subCategories.map(subCategory => <SubCategory key={subCategory.hash}>
  ...
  getMatchedContent(numWordsSurrounding: number): React.ReactNode {
    const {
      query,
      theme,
      contentWithoutHtml
    ...
    return <ContentMatchItem aria-label={textToHighlight} searchWords={[query]} autoEscape sanitize={normalizeSearchString} textToHighlight={textToHighlight} highlightStyle={{
  ...
  renderTitle(): React.ReactNode {
    ...
    return <CategoryListItem>
        <Highlighter searchWords={query ? [query] : []} aria-label={category.title} autoEscape sanitize={normalizeSearchString} highlightStyle={{
        backgroundColor: theme.colors.backgroundColor,
        fontWeight: 'bold'
        ...
        margin: '0 5px',
        fontSize: '12px'
      }}>{this.getMatchedContent(NUM_WORDS_SURROUNDING_MATCH)}</div>
      </CategoryListItem>;
  }
