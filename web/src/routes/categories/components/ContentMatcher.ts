...
class ContentMatcher {
  ...
  getContentBeforeMatchIndex(content: string, matchIndex: number, startOfWord: boolean, numWords: number): string {
    ...
    const limitedMatchBefore = wordsBeforeMatch.slice(-numWords - additionalWordBefore, wordsBeforeMatch.length).join(' ');
  ...
  getMatchedContent(query: string | null | undefined, content: string | null | undefined, numWordsSurrounding: number): string | null | undefined {
    ...
    const contentBefore = this.getContentBeforeMatchIndex(content, matchIndex, queryMatchesStartOfWord, numWordsSurrounding);
