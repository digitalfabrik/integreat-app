...
export const EventsPage = ({
  ...
  const renderEventListItem = () => (event: EventModel) => <EventListItem event={event} formatter={formatter} key={event.path} />;

  const cityModel = cities.find(_cityModel => _cityModel.code === city);

  if (!cityModel || !cityModel.eventsEnabled) {
    return <Failure errorMessage='notFound.category' goToMessage='goTo.categories' goToPath={new CategoriesRouteConfig().getRoutePath({
      city,
      language
    })} />;
  }
  ...
  if (eventId) {
    ...
    if (event) {
      ...
      return <>
          ...
          <Page defaultThumbnailSrc={defaultThumbnail} thumbnailSrcSet={event.featuredImage && featuredImageToSrcSet(event.featuredImage, THUMBNAIL_WIDTH)} lastUpdate={event.lastUpdate} content={event.content} title={event.title} formatter={formatter} onInternalLinkClick={push}>
            <>
...
export default connect<$Diff<PropsType, {
  t: TFunction;
}>, OwnPropsType, _, _, _, _>(mapStateTypeToProps, () => ({}))(withTranslation<PropsType>('events')(EventsPage));