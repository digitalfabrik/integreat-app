...
describe('WohnenOfferPage', () => {
  ...
  it('should render list if no hash is supplied', () => {
    const wohnenPage = shallow(<WohnenOfferPage wohnenOffers={wohnenOffers} city={city} language={language} offers={offers} t={t} />);
    expect(wohnenPage).toMatchSnapshot();
  });
  it('should render detailed offer if hash is supplied', () => {
    const wohnenPage = shallow(<WohnenOfferPage wohnenOffers={wohnenOffers} city={city} language={language} wohnenOfferHash={wohnenOfferHash} offers={[offer]} t={t} />);
    expect(wohnenPage).toMatchSnapshot();
  });
  it('should render failure offer if offer is not found', () => {
    const offersPage = shallow(<WohnenOfferPage wohnenOffers={wohnenOffers} city={city} language={language} wohnenOfferHash='invalid hash' offers={[offer]} t={t} />);
    expect(offersPage).toMatchSnapshot();
  });
  it('should render failure city does not support offer', () => {
    const wohnenPage = shallow(<WohnenOfferPage wohnenOffers={wohnenOffers} city={city} language={language} wohnenOfferHash={wohnenOfferHash} offers={[]} t={t} />);
    expect(wohnenPage).toMatchSnapshot();
  });
