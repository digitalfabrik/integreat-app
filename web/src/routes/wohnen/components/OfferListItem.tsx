...
const Description: StyledComponent<{}, ThemeType, any> = styled.div`
  display: flex;
  flex-direction: column;
`;
const Price: StyledComponent<{}, ThemeType, any> = styled.span`
  align-self: flex-end;
`;
type PropsType = {
...
class OfferListItem extends React.PureComponent<PropsType> {
  render() {
    ...
    if (offer.formData instanceof WohnenFormData) {
      ...
      const offerPath = new WohnenRouteConfig().getRoutePath({
        city,
        language,
        offerHash: hash
      });
      ...
      return <ListItem path={offerPath} title={accommodation.title}>
          <Description>
