...
const Overlay: StyledComponent<{}, ThemeType, any> = styled.div`
  position: absolute;
  top: 0;
  right: 0;
  bottom: 0;
  ...
  opacity: 0.9;
`;
const ModalContainer: StyledComponent<{}, ThemeType, any> = styled.div`
  position: fixed;
  top: 0;
  right: 0;
  bottom: 0;
...
export class FeedbackModal extends React.Component<PropsType, StateType> {
  ...
  renderContent(): React.ReactNode {
    ...
    if (['IDLE', 'ERROR'].includes(sendingStatus)) {
      return <FeedbackBoxContainer isPositiveRatingSelected={feedbackRating === POSITIVE_RATING} onSubmit={this.handleSubmit} sendingStatus={sendingStatus} {...otherProps} />;
    } else {
