...
type PropsType = {
  cities: Array<CityModel> | null | undefined;
  categories: CategoriesMapModel | null | undefined;
...
type LocalStateType = {
  ...
  feedbackModalRating: FeedbackRatingType | null | undefined;
...
export class LocationLayout extends React.Component<PropsType, LocalStateType> {
  state = {
    asideStickyTop: 0,
    feedbackModalRating: null,
    footerClicked: 0
  };
  handleStickyTopChanged = (asideStickyTop: number) => this.setState({
    asideStickyTop
  });
  handleFooterClicked = () => {
    ...
    this.setState(prevState => {
      return { ...prevState,
        footerClicked: prevState.footerClicked + 1
      };
    });
  };

  getCurrentCity(): CityModel | null | undefined {
    const {
      location,
      cities
    } = this.props;
  ...
  renderFeedbackModal = (): React.ReactNode => {
    ...
    return <FeedbackModal feedbackRating={this.state.feedbackModalRating} closeFeedbackModal={this.closeFeedbackModal} location={location} {...feedbackTargetInformation} />;
  };
  openFeedbackModal = (rating: FeedbackRatingType) => this.setState({
    feedbackModalRating: rating
  });
  ...
  renderToolbar = (): React.ReactNode => {
    ...
    const feedbackRoutes = [OFFERS_ROUTE, EVENTS_ROUTE, LOCAL_NEWS_ROUTE, TUNEWS_ROUTE, DISCLAIMER_ROUTE, WOHNEN_ROUTE, SPRUNGBRETT_ROUTE, POIS_ROUTE];

    if (type === CATEGORIES_ROUTE) {
      return <CategoriesToolbar categories={categories} location={location} openFeedbackModal={this.openFeedbackModal} viewportSmall={viewportSmall} />;
  ...
  render() {
    ...
    return <Layout asideStickyTop={this.state.asideStickyTop} header={<LocationHeader cityModel={cityModel} languageChangePaths={languageChangePaths} location={location} viewportSmall={viewportSmall} onStickyTopChanged={this.handleStickyTopChanged} />} footer={!isLoading ? <LocationFooter onClick={this.handleFooterClicked} city={city} language={language} /> : null} toolbar={this.renderToolbar()} modal={type !== SEARCH_ROUTE && this.renderFeedbackModal()} darkMode={darkMode}>
        {children}
      </Layout>;
  }
