...
describe('LocationLayout', () => {
  ...
  const renderLocationLayout = (location, isLoading) => <LocationLayout location={createLocation({ ...location
  })} categories={categories} cities={cities} languageChangePaths={languageChangePaths} feedbackTargetInformation={feedbackTargetInformation} viewportSmall toggleDarkMode={() => {}} darkMode isLoading={isLoading}>
      <MockNode />
    </LocationLayout>;
  ...
  describe('renderToolbar', () => {
    it('should render a CategoriesToolbar if current route is categories', () => {
      ...
      const {
        getByText
      } = render(<ThemeProvider theme={theme}>{renderLocationLayout(location, false)}</ThemeProvider>);
  ...
  it('should not render LocationFooter if loading', () => {
    ...
    const {
      getByText
    } = render(<ThemeProvider theme={theme}>{renderLocationLayout(location, true)}</ThemeProvider>);
    expect(getByText('LocationHeader')).toBeTruthy();
