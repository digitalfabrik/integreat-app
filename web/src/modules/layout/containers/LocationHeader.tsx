...
type PropsType = {
  ...
  languageChangePaths: LanguageChangePathsType | null | undefined;
};
export class LocationHeader extends React.Component<PropsType> {
  getActionItems(): Array<React.ReactNode> {
    const {
      location,
      languageChangePaths,
      t
    ...
    return [<HeaderActionBarItemLink key='search' href={new SearchRouteConfig().getRoutePath({
      city,
      language
    })} text={t('search')} iconSrc={searchIcon} />, ...(!buildConfig().featureFlags.fixedCity ? [<HeaderActionBarItemLink key='location' href={new LandingRouteConfig().getRoutePath({
      language
    })} text={t('changeLocation')} iconSrc={landingIcon} />] : []), <LanguageSelector key='language' languageChangePaths={languageChangePaths} isHeaderActionItem location={location} />];
  }

  getNavigationItems(): Array<React.ReactNode> {
    const {
      t,
      cityModel,
      location
    ...
    const items: Array<React.ReactNode> = [<HeaderNavigationItem key='categories' href={new CategoriesRouteConfig().getRoutePath({
      city,
      language
    })} active={currentRoute === CATEGORIES_ROUTE} text={t('localInformation')} icon={localInformationIcon} />];

    if (isNewsVisible) {
      const newsUrl = pushNotificationsEnabled ? new LocalNewsRouteConfig().getRoutePath({
        city,
        language
      }) : new TunewsRouteConfig().getRoutePath({
        city,
        language
      });
      items.push(<HeaderNavigationItem key='news' href={newsUrl} active={newsRoutes.includes(currentRoute)} text={t('news')} icon={newsIcon} />);
    }

    if (isEventsVisible) {
      items.push(<HeaderNavigationItem key='events' href={new EventsRouteConfig().getRoutePath({
        city,
        language
      })} active={currentRoute === EVENTS_ROUTE} text={t('events')} icon={eventsIcon} />);
    }

    if (isPoisVisible) {
      items.push(<HeaderNavigationItem key='pois' href={new PoisRouteConfig().getRoutePath({
        city,
        language
      })} active={currentRoute === POIS_ROUTE} text={t('pois')} icon={poisIcon} />);
    }

    if (isOffersVisible) {
      items.push(<HeaderNavigationItem key='offers' href={new OffersRouteConfig().getRoutePath({
        city,
        language
      })} active={offersRoutes.includes(currentRoute)} text={t('offers')} icon={offersIcon} />);
    }
  ...
  render() {
    ...
    return <Header viewportSmall={this.props.viewportSmall} logoHref={new CategoriesRouteConfig().getRoutePath({
      city,
      language
    })} actionItems={this.getActionItems()} cityName={cityModel.name} navigationItems={this.getNavigationItems()} onStickyTopChanged={this.props.onStickyTopChanged} />;
  }
