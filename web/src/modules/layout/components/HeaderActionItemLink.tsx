...
const StyledSpan: StyledComponent<{}, ThemeType, any> = StyledLink.withComponent('span');
type PropsType = {
  href?: string;
  text: string;
  iconSrc: string;
...
const HeaderActionItemLink = ({
  ...
  return <Tooltip text={text} flow='down' smallViewportFlow='left'>
      {href ? <StyledLink to={href} aria-label={text}>
          <img alt='' src={iconSrc} />
        </StyledLink> : <StyledSpan aria-label={text}>
          <img alt='' src={iconSrc} />
        </StyledSpan>}
    </Tooltip>;
};
