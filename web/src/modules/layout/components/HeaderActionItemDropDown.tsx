...
export const Container: StyledComponent<{}, ThemeType, any> = styled.div`
...
`;
type PropsType = {
  children: (closeDropDown: () => void) => React.ReactElement<any>;
  iconSrc: string;
  text: string;
};
...
const HeaderActionItemDropDown = (props: PropsType) => {
  ...
  return <Container ref={wrapperRef}>
      ...
      <DropDownContainer aria-label={dropDownActive} active={dropDownActive} // We need to have th visibility here, else the jest-dom testing library can not assert on it
    style={{
