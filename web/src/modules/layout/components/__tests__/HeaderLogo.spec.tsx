...
describe('HeaderLogo', () => {
  ...
  it('should show the regular header app icon if there is no campaign', () => {
    clock = lolex.install({
      now: 1615374110000,
      toFake: []
    }); // Wed Mar 10 2021 11:01:50 GMT+0000

    config.campaign = undefined;
    config.icons.appLogo = '/my-regular-logo';
    const {
      getByAltText
    } = render(<HeaderLogo link='https://example.com' />);
    // eslint-disable-next-line flowtype/no-weak-types
    expect(((getByAltText('IntegreatTestCms') as any) as HTMLMediaElement).src).toContain(config.icons.appLogo);
  });
  it('should show the campaign logo if the current date is between start and end date', () => {
    clock = lolex.install({
      now: 1615374110000,
      toFake: []
    }); // Wed Mar 10 2021 11:01:50
    ...
    // eslint-disable-next-line flowtype/no-weak-types
    expect(((getByAltText('IntegreatTestCms') as any) as HTMLMediaElement).src).toContain(womensDayCampaign.campaignAppLogo);
  });
  it('should show the regular logo if the current date is before the start date', () => {
    clock = lolex.install({
      now: 1614942686000,
      toFake: []
    }); // Fri Mar 05 2021 11:11:26
