...
describe('fetchData', () => {
  ...
  const endpoint = new EndpointBuilder(defaultName).withParamsToUrlMapper(defaultMapParamsToUrl).withMapper(defaultJsonMapper).build();
  let clock;
  const mockedTime = 0;
  beforeEach(() => {
    clock = lolex.install({
  ...
  it('should fail if json is malformed', async () => {
    ...
    const data = await fetchData(endpoint, dispatch, oldPayload, params);
    const mappingError = new MappingError(defaultName, 'invalid json response body at  reason: Unexpected token I in JSON at position 0');
  ...
  it('should use overrideResponse correctly', async () => {
    const json = [];
    const endpoint = new EndpointBuilder<any, CityModel[]>(defaultName).withParamsToUrlMapper(defaultMapParamsToUrl).withMapper(defaultJsonMapper).withResponseOverride(json).build();
    const dispatch = jest.fn();
  ...
  it('should use overrideError correctly', async () => {
    ...
    const endpoint = new EndpointBuilder<any, CityModel[]>(defaultName).withParamsToUrlMapper(defaultMapParamsToUrl).withMapper(defaultJsonMapper).withResponseOverride([]).withErrorOverride(error).build();
    const dispatch = jest.fn();
    const oldPayload = new Payload<CityModel[]>(false);
    const params = {
      var1: 'a',
