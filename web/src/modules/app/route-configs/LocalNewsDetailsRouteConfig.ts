...
import { createCitiesEndpoint, createEventsEndpoint, createLocalNewsElementEndpoint, createLanguagesEndpoint, LocalNewsModel, Payload, CityModel } from "api-client";
import fetchData from "../fetchData";
import { cmsApiBaseUrl } from "../constants/urls";
import type { StateType } from "../StateType";
type LocalNewsDetailsType = {
  city: string;
  language: string;
  id: number;
};
...
const localNewsDetailsRoute: Route = {
  ...
  thunk: async (dispatch, getState) => {
    ...
    await Promise.all([fetchData(createLocalNewsElementEndpoint(cmsApiBaseUrl), dispatch, state.localNewsElement, {
      city,
      language,
      id: newsId
    }), fetchData(createCitiesEndpoint(cmsApiBaseUrl), dispatch, state.cities), fetchData(createEventsEndpoint(cmsApiBaseUrl), dispatch, state.events, {
      city,
      language
...
class LocalNewsDetailsRouteConfig implements RouteConfig<LocalNewsDetailsType, RequiredPayloadsType> {
  ...
  getRoutePath = ({
    ...
    language,
    id
  }: LocalNewsDetailsType): string => `/${city}/${language}/news/local/${id}`;
