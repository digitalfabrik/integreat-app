...
import { CategoriesMapModel, CityModel, createCategoriesEndpoint, createCitiesEndpoint, createEventsEndpoint, createLanguagesEndpoint, Payload } from "api-client";
import { cmsApiBaseUrl } from "../constants/urls";
import type { StateType } from "../StateType";
type SearchRouteParamsType = {
  city: string;
  language: string;
};
...
const searchRoute: Route = {
  ...
  thunk: async (dispatch, getState) => {
    ...
    await Promise.all([fetchData(createCitiesEndpoint(cmsApiBaseUrl), dispatch, state.cities), fetchData(createEventsEndpoint(cmsApiBaseUrl), dispatch, state.events, {
      ...
      city,
      language
    }), fetchData(createCategoriesEndpoint(cmsApiBaseUrl), dispatch, state.categories, {
      city,
      language
    })]);
  }
...
class SearchRouteConfig implements RouteConfig<SearchRouteParamsType, RequiredPayloadsType> {
  ...
  getRoutePath = ({
    city,
    language
  }: SearchRouteParamsType): string => `/${city}/${language}/search`;
  ...
  getPageTitle = ({
    cityName,
    t
  }) => cityName ? `${t('pageTitles.search')} - ${cityName}` : null;
