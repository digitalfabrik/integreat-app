...
import { TunewsModel, Payload, createTunewsElementEndpoint, createCitiesEndpoint, createEventsEndpoint, CityModel } from "api-client";
import fetchData from "../fetchData";
import { cmsApiBaseUrl, tunewsApiBaseUrl } from "../constants/urls";
import type { StateType } from "../StateType";
type TunewsDetailsRouteParamsType = {
  city: string;
  language: string;
  id: number;
};
...
const tunewsDetailsRoute: Route = {
  ...
  thunk: async (dispatch, getState) => {
    ...
    await Promise.all([fetchData(createTunewsElementEndpoint(tunewsApiBaseUrl), dispatch, state.tunewsElement, {
      city,
      language,
      id: newsId
...
class TunewsDetailsRouteConfig implements RouteConfig<TunewsDetailsRouteParamsType, RequiredPayloadsType> {
  ...
  getRoutePath = ({
    ...
    language,
    id
  }): string => `/${city}/${language}/news/tu-news/${id}`;
