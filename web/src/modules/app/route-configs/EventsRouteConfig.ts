...
import { CityModel, createCitiesEndpoint, createEventsEndpoint, createLanguagesEndpoint, EventModel, Payload } from "api-client";
import fetchData from "../fetchData";
import { cmsApiBaseUrl } from "../constants/urls";
import type { StateType } from "../StateType";
type EventsRouteParamsType = {
  city: string;
  language: string;
};
...
class EventsRouteConfig implements RouteConfig<EventsRouteParamsType, RequiredPayloadsType> {
  ...
  getRoutePath = ({
    city,
    language
  }: EventsRouteParamsType): string => `/${city}/${language}/events`;
