...
import { createCitiesEndpoint, createEventsEndpoint, createOffersEndpoint, createLanguagesEndpoint, createWohnenEndpoint, WOHNEN_OFFER, OfferModel, Payload, WohnenOfferModel } from "api-client";
...
type RequiredPayloadsType = {
  wohnenOffers: Payload<Array<WohnenOfferModel>>;
...
const fetchOffers = async (dispatch, getState) => {
  ...
  const offers: Array<OfferModel> | null | undefined = offersPayload.data;
  ...
  if (offers) {
    ...
    if (offer && offer.postData) {
      const params = {
        city: offer.postData.get('api-name')
      };
...
class WohnenRouteConfig implements RouteConfig<RouteParamsType, RequiredPayloadsType> {
  ...
  getRoutePath = ({
    ...
    language,
    offerHash
  }: RouteParamsType): string => `/${city}/${language}/offers/${WOHNEN_OFFER}${offerHash ? `/${offerHash}` : ''}`;
