...
import { createCitiesEndpoint, createEventsEndpoint, createLocalNewsEndpoint, createLanguagesEndpoint, LocalNewsModel, Payload, CityModel } from "api-client";
import fetchData from "../fetchData";
import { cmsApiBaseUrl } from "../constants/urls";
import type { StateType } from "../StateType";
type LocalNewsRouteParamsType = {
  city: string;
  language: string;
};
...
const localNewsRoute: Route = {
  ...
  thunk: async (dispatch, getState) => {
    ...
    await Promise.all([fetchData(createLocalNewsEndpoint(cmsApiBaseUrl), dispatch, state.localNews, {
      city,
      language
...
class LocalNewsRouteConfig implements RouteConfig<LocalNewsRouteParamsType, RequiredPayloadsType> {
  ...
  getRoutePath = ({
    city,
    language
  }: LocalNewsRouteParamsType): string => `/${city}/${language}/news/local`;
