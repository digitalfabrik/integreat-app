...
import { createCitiesEndpoint, createEventsEndpoint, createOffersEndpoint, createLanguagesEndpoint, createSprungbrettJobsEndpoint, SPRUNGBRETT_OFFER, OfferModel, Payload, SprungbrettJobModel } from "api-client";
import type { Route } from "redux-first-router";
import fetchData from "../fetchData";
import { cmsApiBaseUrl } from "../constants/urls";
import type { StateType } from "../StateType";
type SprungbrettRouteParamsType = {
  city: string;
  language: string;
};
...
class SprungbrettRouteConfig implements RouteConfig<SprungbrettRouteParamsType, RequiredPayloadsType> {
  ...
  getRoutePath = ({
    city,
    language
  }: SprungbrettRouteParamsType): string => `/${city}/${language}/offers/${SPRUNGBRETT_OFFER}`;
