...
import { createCitiesEndpoint, createDisclaimerEndpoint, createEventsEndpoint, createLanguagesEndpoint, PageModel, Payload } from "api-client";
import { cmsApiBaseUrl } from "../constants/urls";
import type { StateType } from "../StateType";
type DisclaimerRouteParamsType = {
  city: string;
  language: string;
};
...
const disclaimerRoute: Route = {
  ...
  thunk: async (dispatch, getState) => {
    ...
    await Promise.all([fetchData(createCitiesEndpoint(cmsApiBaseUrl), dispatch, state.cities), fetchData(createEventsEndpoint(cmsApiBaseUrl), dispatch, state.events, {
      ...
      city,
      language
    }), fetchData(createDisclaimerEndpoint(cmsApiBaseUrl), dispatch, state.disclaimer, {
      city,
      language
    })]);
  }
...
class DisclaimerRouteConfig implements RouteConfig<DisclaimerRouteParamsType, RequiredPayloadsType> {
  ...
  getPageTitle = ({
    t,
    cityName
  }) => cityName ? `${t('pageTitles.disclaimer')} - ${cityName}` : null;
  getRoutePath = ({
    city,
    language
  }: DisclaimerRouteParamsType): string => `/${city}/${language}/disclaimer`;
