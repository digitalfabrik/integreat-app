...
export type StartFetchMoreActionType<T extends PayloadDataType> = {
  type: string;
  payload: Payload<T>;
};
export const startFetchMoreActionName = (stateName: string): string => `START_FETCH_MORE_${stateName.toUpperCase()}`;

const startFetchMoreAction = <T extends PayloadDataType>(stateName: string, formattedUrl: string): StartFetchMoreActionType<T> => {
