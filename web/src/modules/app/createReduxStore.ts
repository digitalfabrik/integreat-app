...
const createReduxStore = (initialState: StateType = {}, routesMap: RoutesMap = defaultRoutesMap): Store<StateType, StoreActionType> => {
  const {
    reducer,
    middleware,
    enhancer
   ...
   */
  const middlewares = [middleware, thunkMiddleware // Allows to return functions in actions
  ];
  ...
  const rootReducer = combineReducers({ ...endpointReducers,
