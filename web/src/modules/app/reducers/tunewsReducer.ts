...
type TunewsFetchActionType = StartFetchActionType<{
  language: string;
}> | FinishFetchActionType<Array<TunewsModel>, any>;
const defaultState = {
  allData: [],
  language: null,
  hasMore: true,
...
const tunewsReducer: ReduxReducer<TunewsStateType, TunewsFetchActionType> = (state: TunewsStateType = defaultState, action: TunewsFetchActionType) => {
  switch (action.type) {
