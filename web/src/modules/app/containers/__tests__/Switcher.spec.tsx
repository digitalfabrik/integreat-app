...
import { CategoriesMapModel, CategoryModel, DateModel, EventModel, OfferModel, LanguageModel, LocationModel, PageModel, Payload, PoiModel, SprungbrettJobModel, WohnenFormData, WohnenOfferModel } from "api-client";
...
describe('Switcher', () => {
  ...
  const events = [new EventModel({
    ...
    availableLanguages: new Map([['de', '/augsburg/de/events/nulltes_event'], ['ar', '/augsburg/ar/events/nulltes_event']]),
    lastUpdate: moment(0),
    hash: '2fe6283485a93932'
  })];
  const cities = new CityModelBuilder(1).build();
  const languages = [new LanguageModel('de', 'Deutsch'), new LanguageModel('en', 'English')];
  const sprungbrettJobs = [new SprungbrettJobModel({
    id: 0,
    title: 'WebDeveloper',
    location: 'Augsburg',
    isEmployment: true,
  ...
  const wohnenOffers = [new WohnenOfferModel({
    email: 'mail@mail.com',
  ...
  const createSwitcher = (currentRoute: string, pathname?: string): React.ReactElement<any> => {
    ...
    return <Switcher viewportSmall={false} location={location} citiesPayload={citiesPayload} categoriesPayload={categoriesPayload} eventsPayload={eventsPayload} offersPayload={offersPayload} localNewsPayload={localNewsPayload} localNewsElementPayload={localNewsElementPayload} tunewsPayload={tunewsPayload} tunewsElementPayload={tunewsElementPayload} poisPayload={poisPayload} disclaimerPayload={disclaimerPayload} languagesPayload={languagesPayload} tunewsLanguagesPayload={tunewsLanguagesPayload} t={t} sprungbrettJobsPayload={sprungbrettPayload} wohnenOffersPayload={wohnenOffersPayload} darkMode toggleDarkMode={toggleDarkMode} />;
  };
