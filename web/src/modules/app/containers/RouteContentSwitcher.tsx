...
type PropsType = {
  ...
  payloads: Record<string, Payload<any>>;
  isLoading: boolean;
};

class RouteContentSwitcher extends React.PureComponent<PropsType> {
  renderFailureLoadingComponents = (): React.ReactNode => {
    const {
      isLoading,
      payloads
    } = this.props;
  ...
  isWaitingForOffer = (offerType: "wohnenOffer" | "sprungbrettJobs", payloads: Record<string, Payload<any>>): boolean => {
  ...
  renderOfferFailure = (): React.ReactNode => {
    const {
      payloads,
      location
    } = this.props;
  ...
  render() {
    ...
    return this.renderOfferFailure() || this.renderFailureLoadingComponents() || <RouteContent {...reduce(payloads, (result, value, key: string) => ({
      [key]: value.data,
      ...result
    }), {})} />;
  }
