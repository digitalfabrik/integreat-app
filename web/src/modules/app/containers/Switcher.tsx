...
import { CategoriesMapModel, CityModel, EventModel, OfferModel, LanguageModel, LocalNewsModel, PageModel, Payload, PoiModel, TunewsModel, WohnenOfferModel } from "api-client";
...
export type LanguageChangePathsType = Array<{
  code: string;
  path: string | null;
  name: string;
}>;
...
export class Switcher extends React.Component<PropsType> {
  ...
  getLanguageChangePaths = (routeConfig: RouteConfig<any, any>): LanguageChangePathsType | null | undefined => {
    const {
      languagesPayload,
      location
    } = this.props;
    ...
    return languagesPayload.data && languagesPayload.data.map(language => ({
      path: routeConfig.getLanguageChangePath({
        payloads,
        location,
  ...
  renderHelmet = (): React.ReactNode => {
    ...
    const pageTitle = routeConfig.getPageTitle({
      t,
      payloads,
    ...
    return <Helmet pageTitle={pageTitle} metaDescription={metaDescription} languageChangePaths={languageChangePaths} cityModel={cityModel} />;
  };
  ...
  renderLayoutWithContent(): React.ReactNode {
    const {
      location,
      viewportSmall,
      darkMode,
    ...
    const routeConfig = getRouteConfig(location.type);
    const payloads = routeConfig.getRequiredPayloads(this.getAllPayloads());
    const feedbackTargetInformation = routeConfig.getFeedbackTargetInformation({
      location,
      payloads
    });
    const languageChangePaths = this.getLanguageChangePaths(routeConfig);
    ...
    if (invalidCity || invalidLanguage || !routeConfig.isLocationLayoutRoute) {
      ...
      return <Layout footer={showFooter && <GeneralFooter language={language} />} header={showHeader && <GeneralHeader viewportSmall={viewportSmall} />} darkMode={darkMode}>
          {invalidCity ? <FailureSwitcher error={new Error('notFound.category')} /> : invalidLanguage && cities ? <LanguageFailure cities={cities} location={location} languageChangePaths={languageChangePaths || []} /> : <RouteContentSwitcher location={location} payloads={payloads} isLoading={isLoading} />}
        </Layout>;
    } else {
      return <LocationLayout feedbackTargetInformation={feedbackTargetInformation} location={location} categories={categoriesPayload.data} cities={citiesPayload.data} darkMode={darkMode} viewportSmall={viewportSmall} toggleDarkMode={toggleDarkMode} languageChangePaths={languageChangePaths} isLoading={isLoading}>
...
export default connect<$Diff<PropsType, {
  t: TFunction;
}>, {}, _, _, _, _>(mapStateToProps, mapDispatchToProps)(withTranslation<PropsType>('app')(Switcher));