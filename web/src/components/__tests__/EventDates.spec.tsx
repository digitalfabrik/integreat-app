...
describe('EventDates', () => {
  ...
  it('should reveal the upcoming dates when expanding a monthly recurring event', async () => {
    ...
    await waitFor(() => expect(getByText('13. Nov. - 14. Nov.')).toBeVisible())
    expect(getByText('11. Dez. - 12. Dez.')).toBeVisible()
    expect(getByText('8. Jan. 2024 - 9. Jan. 2024')).toBeVisible()
    expect(getByText('12. Feb. 2024 - 13. Feb. 2024')).toBeVisible()
  ...
  it('should reveal the upcoming dates when expanding a weekly recurring event', async () => {
    ...
    await waitFor(() => expect(getByText('16. Okt. - 17. Okt.')).toBeVisible())
    expect(getByText('13. Nov. - 14. Nov.')).toBeVisible()
    expect(getByText('…')).toBeVisible()
  })

  it('should reveal fewer dates in short format on mobile', async () => {
    mocked(useDimensions).mockImplementation(() => ({ ...mockDimensions, mobile: true }))
    const event = eventWithDate('DTSTART:20230414T050000\nRRULE:FREQ=MONTHLY;BYDAY=+2MO')
    const { getByRole, getByText, queryByText } = renderWithTheme(<EventDates event={event} languageCode={language} />)

    fireEvent.click(getByRole('button'))

    await waitFor(() => expect(getByText('8. Jan. 2024 - 9. Jan. 2024')).toBeVisible())
  ...
  it('should show each further date with its own time when times vary', async () => {
    ...
    await waitFor(() => expect(getByText(/18\. Okt\. - 19\. Okt\..*9:00 - 11:00/)).toBeVisible())
    expect(getByText(/20\. Okt\. - 21\. Okt\..*13:00 - 15:00/)).toBeVisible()
  })
})
