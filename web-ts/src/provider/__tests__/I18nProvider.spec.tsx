...
import BrowserLanguageDetectorService from '../../service/__mocks__/BrowserLanguageDetectorService'
...
describe('I18nProvider', () => {
  it('should choose the browser language', async () => {
    BrowserLanguageDetectorService.detect.mockReturnValue(['ar'])
    ...
    act(() => {
      render(<Provider store={store}>
        <I18nProvider>
          <Translation>{(t, {
            i18n
          }) => <p>{i18n.languages[0]}</p>}</Translation>
        </I18nProvider>
      </Provider>)
    })
  ...
  it('should use fallbacks for ui translations', async () => {
    BrowserLanguageDetectorService.detect.mockReturnValue(['ku'])
    const store = prepareStore()
    act(() => {
      render(<Provider store={store}>
        <I18nProvider>
          <Translation>{(t, {
            i18n
          }) => <p>{t('dashboard:localInformation')}</p>}</Translation>
        </I18nProvider>
      </Provider>)
    })
  ...
  it('should choose the default fallback for ui translations', async () => {
    BrowserLanguageDetectorService.detect.mockReturnValue(['en'])
    const store = prepareStore()
    act(() => {
      render(<Provider store={store}>
        <I18nProvider>
  ...
  it('should support language tags with dashes', async () => {
    BrowserLanguageDetectorService.detect.mockReturnValue(['zh-hans'])
    const store = prepareStore()
    act(() => {
      render(<Provider store={store}>
        <I18nProvider>
